...
import { BadgeCheck, Trophy, Star, Award, Coins } from "lucide-react";
import { useEffect, useState } from "react";
import { getNextRank } from "@/services/rankService"; // Ajusta el path según tu estructura
...
export const CardDisplay = ({ user }) => {
  ...
  const [totalPurchases, setTotalPurchases] = useState(0); // Corregido el nombre
  ...
  useEffect(() => {
    ...
    fetchNextRank();
    fetchTotalPurchases(); // Corregido la llamada
    ...
    <div className="bg-card rounded-xl p-8 space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div className="bg-secondary/50 p-4 rounded-lg text-center">
          ...
          <p className="text-lg font-semibold">{totalPurchases}</p> {/* Corregido el nombre */}
