...
export const TransactionsTable = ({ purchases, loading, onPurchaseUpdate }) => {
  ...
  const processUserOrders = async () => {
    try {
      ...
      const userOrders = purchases.filter(order => order.userId._id === user._id);
      
      // Ordenar por fecha de compra (más reciente primero)
      const sortedOrders = [...userOrders].sort((a, b) => {
        // Convertir las fechas a timestamps (números) antes de restar
        const dateA = new Date(a.purchaseDate || a.createdAt).getTime();
        const dateB = new Date(b.purchaseDate || b.createdAt).getTime();
        return dateB - dateA; // Ordenado de más reciente a más antiguo
      });
      
      setOrders(sortedOrders);

      const symbolsPromises = sortedOrders.map(order =>
        getCurrencyById(order.currencyId)
          .then(currency => ({ [order._id]: currency.symbol }))
          .catch(() => ({ [order._id]: '' }))
      );

      const symbolsArray = await Promise.all(symbolsPromises);
      const symbolsMap = symbolsArray.reduce((acc, curr) => ({ ...acc, ...curr }), {});
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 space-y-4 md:space-y-0">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Mira todos tus pedidos y sus estados aqui
            </p>
          </div>
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="text-muted-foreground text-sm border-b border-border">
                    ...
                    <th className="text-center pb-4 font-normal px-4 md:px-6">TOTAL(RP)</th>
                    <th className="text-center pb-4 font-normal px-4 md:px-6">TOTAL()</th>
                    ...
                    orders.map((order) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-4 text-center px-4 md:px-6 text-muted-foreground">{order.totalRP}</td>
                          <td className="py-4 text-center px-4 md:px-6 text-muted-foreground">
                            {currencySymbols[order._id]}{order.Total}
                          </td>
                          <td className="py-4 text-center px-4 md:px-6 text-muted-foreground">
